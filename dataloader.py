...
class CollateFn(object):
    def __init__(self, source_tokenizer: Tokenizer, target_tokenizer: Tokenizer):
    ...
    def __call__(self, batch):
        ...
        for (source, target) in batch:
            ...
            target_tokenized  = [Tokenizer.SpecialTokens.START.id,]
            target_tokenized += self.target_tokenizer.tokenize(target)[0]
            target_tokenized += [Tokenizer.SpecialTokens.END.id,]
        ...
        source_tokenized_padded = Tokenizer.pad(source_tokenized_list, max_len_source)
        target_tokenized_padded = Tokenizer.pad(target_tokenized_list, max_len_target)

        source_pad_mask = Tokenizer.pad_mask(source_tokenized_padded)
        target_pad_mask = Tokenizer.pad_mask(target_tokenized_padded)
